...
Fill in the blanks...
```{r}
germinated <- germinated %>% ungroup() %>%
  expand(bed, row=1:(max(germinated$row)+2), column) %>% 
...
plants %>% arrange(parent.pop, mf, rep) %>%
  select(parent.pop, mf, rep, bed, row, column, everything() ) %>% write_sheet("https://docs.google.com/spreadsheets/d/1EaSA2QrlWAjFR35pedcvheQp3X7Agsla6pZtE2C1QJ4",
                                                        sheet="New Bed Map")
```
...
pdf("../output/bedmaps.pdf",width = 8.5, height = 11)
...
pdf("../output/bedmaps2.pdf",width = 8.5, height = 11)

for(b in unique(plan3_plants$block)) {
  germlabel <- ifelse(b %in% germblocks, " germinated", " NOT germinated")
  pl <- plan3_plants %>% filter(block==b) %>%
    ggplot(aes(x0=column, y0=row, label=label, r=.5)) +
  #  geom_circle(fill="lightgreen", alpha=.25, color="grey70") + 
    geom_label(aes(x=column, y=row), size=3.5, lineheight=.8) +
    coord_equal() + 
    theme(axis.text.x = element_text(angle=90, size=12, vjust = 0.5)) +
    ggtitle(str_c("Block ", b, germlabel)) +
    scale_y_continuous(breaks = 1:44) 
    #scale_x_continuous( labels=c("A","B","C","D"))
  print(pl)
}
