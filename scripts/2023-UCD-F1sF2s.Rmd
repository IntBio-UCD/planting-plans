...
extra <- read_sheet(url, 
  ...
  mutate(across(.cols= c(dame, sire), .fns = str_remove, "( |-)[^)]*$" ),
         pop.id = str_c(dame, " x ", sire ),
         unique.ID=seq(from=2001, length.out=n()),
         rack = NaN) %>%
   select(-dame, -sire, rep = `extra rep number`)
...
combined <- combined %>% 
  ...
  group_by(block) %>%
  mutate(row=ifelse(str_sub(block,2,2) == "2" & (length(row[row==min(row)]) < columns), row+1, row)) %>% # adjusts for incomplete rows in first block in each bed
