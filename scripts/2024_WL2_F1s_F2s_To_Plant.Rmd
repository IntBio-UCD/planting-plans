...
F1s_plant <- F1s_short_summarize_twenty %>% filter(Sum_Seeds>20) %>% 
  mutate(plantable_seeds = Sum_Seeds - 20,
         seeds_to_plant = min(plantable_seeds, 26)
         ) %>% 
  arrange(desc(Sum_Seeds))

F1s_plant

sum(F1s_plant$seeds_to_plant)

F1s_plant2 <- F1s_short_summarize_twenty %>% filter(Sum_Seeds>20) %>% 
  mutate(plantable_seeds = Sum_Seeds - 20,
         seeds_to_plant = ifelse(plantable_seeds<26, plantable_seeds, 26) #changed this to an ifelse statement b/c the code above wasn't always giving 26 plantable seeds when possible
         ) %>% 
  arrange(desc(Sum_Seeds))
F1s_plant2
sum(F1s_plant2$seeds_to_plant)
```
...
F2s_short_after_ripened_summarize <- F2s_short_after_ripened %>% 
  ...
  mutate(Seeds_to_Plant=ifelse(Sum_Seeds <= 40, Sum_Seeds, 40)) %>% 
  arrange(Sum_Seeds) 
...
F2s_short_after_ripened_summarize_TM2 <- F2s_short_after_ripened_summarize %>% 
...
F2s_short_after_ripened_summarize_TM2
