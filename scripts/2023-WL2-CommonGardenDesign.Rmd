...
germinated_lowrep <- germinated %>%
  group_by(parent.pop, mf) %>%
  mutate(reps=n()) %>%
  filter(reps < 13) %>% # because 14 blocks and 14 represents full replication
  group_by(parent.pop) %>%
  mutate(block= {map(1:ceiling(n()/13), ~ sample(germblocks)) %>%
...
columns <- 4 # per bed
rows <- 60 # four of which are buffers
...
```{r}
bedwidths   <- seq(1500, 750, length.out=exp.beds )
plan3 <- plan3 %>%
...
```
Fill in the blanks...(not filling in buffers for WL2 becuase spacing is uncertain)
```{r}
