...
germinated <- germinated %>% 
  ...
  # mutate(row=rep(1:(ceiling(n()/columns)), each=columns, length.out=n())) %>% #no buffer rows
    mutate(row=rep(2:(1+ceiling(n()/columns)), each=columns, length.out=n())) %>% #use if you want buffer rows
...
#  expand(row=1:max(row), column) #no buffer rows.  But should fill ragged edges.
   expand(row=1:max(row+1), column) # use this if you want buffer rows
...
```
room for 39 buffer rows total.

2 at the end of each bed.  2 at the beginning of the first 6 rows and 1 at the beginning of the remaining 5.
```{r}
plan3 <- plan3 %>%
  
  #top buffers:
  mutate(buffer=(row<=2 & bed %in% letters[1:6]) | (row <=1 & bed %in% letters[7:exp.beds]), # if you want differnt number of buffers in last row
...
plan3_plants <- plan3_plants %>%
  ...
  mutate(label=ifelse(buffer, "buff", str_c(parent.pop, "\n", mf, "-", rep) ),
         ...
         colorblock=ifelse(parent.pop=="buffer", "buffer", colorblock))
```
