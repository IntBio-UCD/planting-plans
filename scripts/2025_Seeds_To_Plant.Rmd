...
### F1s Merge
```{r}
F1s_all <- full_join(F1s_short, F1s_update_short_year_corrected) 
#Warning: Detected an unexpected many-to-many relationship between `x` and `y`.
#Some merge issues, should check this

F1s_all %>% filter(!is.na(duplicate))
F1s_all %>% filter(is.na(seeds_avail))
F1s_all %>% filter(dame_pop=="WL2", sire_pop=="BH")
F1s_all %>% filter(dame_pop=="BH", sire_pop=="WL2")
...
sum(F1s_plant$Sum_Seeds) #955 total seeds available
...
sum(F1s_plant_summarize_WL2$Sum_Seeds) #403 WL2 F1 seeds 
```
