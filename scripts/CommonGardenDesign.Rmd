...
### Example plot for "30 in veg crop" bed.

Sarah says this is 30 cm of plantable width.  

2041 plants.  1 block per rep, so 13 blocks each of 157 plants

157/4 = 39.25 with 4 columns this is 39.25 plants per column per block.  Call it 40.  But then two more buffers on each side, call it 48
...
plan3 <- plan3 %>%
  mutate(plantedcol=(match(block, LETTERS)-1)*4 + column,
         bedcol = floor((plantedcol -1)*expandfactor + 1   )) 
...
size <- 30 # plant diameter
radius <- plantsize/2 
blankevery <- 3 # what is the spacing between blank beds?  (2 means every other; 3 means every third)
expandfactor <- blankevery/(blankevery-1)
